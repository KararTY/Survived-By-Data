...
var fileNameMap = require(path.join(__dirname, patchDate, 'map.json'))
...
if (!fs.existsSync(path.join(__dirname, patchDate, 'Parsed'))) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed'))
} else {
  rimraf.sync(path.join(__dirname, patchDate, 'Parsed'))
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed'))
}
...
var folder = {
  'ItemDefinition': path.join(__dirname, patchDate, 'ItemDefinition'),
  'Monster': path.join(__dirname, patchDate, 'Monster'),
  'LootTable': path.join(__dirname, patchDate, 'LootTable'),
  'Other': path.join(__dirname, patchDate, 'Other'),
  'Ancestral': path.join(__dirname, patchDate, 'Ancestral'),
  'CraftingRecipe': path.join(__dirname, patchDate, 'CraftingRecipe'),
  'ItemModifier': path.join(__dirname, patchDate, 'ItemModifier'),
  'LootBox': path.join(__dirname, patchDate, 'LootBox'),
  'NPC': path.join(__dirname, patchDate, 'NPC'),
  'Player': path.join(__dirname, patchDate, 'Player'),
  'Challenge': path.join(__dirname, patchDate, 'Challenge')
}

let folderName1 = 'ItemDefinition'
if (folder[folderName1]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName1))
  fs.readdirSync(folder[folderName1]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName1, `${itemDefinition.name}.json`)
...
if (folder[folderName2]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName2))
  fs.readdirSync(folder[folderName2]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName2, `${lootTable.from}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(lootTable)
      fs.writeFileSync(filename, JSON.stringify(file))
...
if (folder[folderName3]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName3))
  fs.readdirSync(folder[folderName3]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName3, `${monsterInfo.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
...
if (folder[folderName4]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName4))
  fs.readdirSync(folder[folderName4]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName4, `${ancestral.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(ancestral)
      fs.writeFileSync(filename, JSON.stringify(file))
...
if (folder[folderName5]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName5))
  fs.readdirSync(folder[folderName5]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName5, `${craftingRecipe.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
...
if (folder[folderName7]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName7))
  fs.readdirSync(folder[folderName7]).forEach(val => {
    ...
    var itemModifier = {
      ...
      data: (function () {
        ...
        var f = require(path.join(folder['Other'], fileMap(file['0 MonoBehaviour Base']['0 Array availableModifiers'][0]['0 PPtr<$GameObject> data']['0 int m_FileID']) + file['0 MonoBehaviour Base']['0 Array availableModifiers'][0]['0 PPtr<$GameObject> data']['0 SInt64 m_PathID'] + '.json'))
        return f['0 GameObject Base']['0 vector m_Component']['0 Array Array'].map(v => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName7, `${itemModifier.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
...
if (folder[folderName8]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName8))
  fs.readdirSync(folder[folderName8]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName8, `${lootBox.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(lootBox)
      fs.writeFileSync(filename, JSON.stringify(file))
...
if (folder[folderName9]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName9))
  fs.readdirSync(folder[folderName9]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName9, `${npc.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(npc)
      fs.writeFileSync(filename, JSON.stringify(file))
...
if (folder[folderName10]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName10))
  fs.readdirSync(folder[folderName10]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName10, `${player.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(player)
      fs.writeFileSync(filename, JSON.stringify(file))
...
if (folder[folderName11]) {
  fs.mkdirSync(path.join(__dirname, patchDate, 'Parsed', folderName11))
  fs.readdirSync(folder[folderName11]).forEach(val => {
    ...
    var filename = path.join(__dirname, patchDate, 'Parsed', folderName11, `${challenge.name}.json`)
    if (fs.existsSync(filename)) {
      var file = JSON.parse(fs.readFileSync(filename, 'utf-8'))
      file.push(challenge)
      fs.writeFileSync(filename, JSON.stringify(file))
